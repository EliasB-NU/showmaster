...
func (a *API) login(c *fiber.Ctx) error {
	...
	err = a.PSQL.Where("user_id = ? AND device_id = ?", user.ID, data.DeviceId).Find(&browserTokens).Error
	...
	if len(browserTokens) != 0 {
		log.Printf("User is already logged in on that device: %d\n", user.ID)
	...
	err = a.PSQL.Where("user_id = ?", user.ID).First(&perms).Error
	if err != nil {
...
func (a *API) checkIfUserIsLoggedIn(c *fiber.Ctx) error {
	...
	var browserToken database.BrowserToken
	err = a.PSQL.Preload("User").Where("device_id = ? AND token = ?", data.DeviceId, data.Token).Find(&browserToken).Error
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return c.Status(fiber.StatusForbidden).JSON("user is not logged in")
		}
	...
	// Get user permissions
	var perms database.Permission
	err = a.PSQL.Where("user_id = ?", browserToken.UserID).First(&perms).Error
	if err != nil {
		log.Printf("Error getting user permissions: %v\n", err)
		return c.Status(fiber.StatusInternalServerError).JSON("Error getting user permissions")
	}
